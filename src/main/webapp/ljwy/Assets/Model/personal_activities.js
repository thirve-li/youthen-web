//个人活动
var personal_activities = function() {
	this.all_activity = function() {
		var isCookie = JSON.parse($.cookie("IsAllActivity"));
		if (isCookie == null) {
			getBomBbox("↑上拉加载更多");
			$.cookie("IsAllActivity", 1, {
				expires: 3650,
				path: "/"
			});
		}
		...
		$.post(interfaceApi, data, function(result) {
			...
			if (result.messageCode == 1) {
				...
				$.each(result.resultObject, function(i, item) {
					$('<div class="topic"><a href="personal_activities_details.html?id=' + item.id + '"><div class="topic_img"><img src="' + imageNull(item.createrImage) + '"/></div><div class="topic_content"><p class="title">' + item.name + '</p><p><span class="nickname">' + item.createrNickName + '</span><span class="createtime">' + item.createTime + '</span></p></div></a></div>').appendTo(".list");
	...
	this.activity_detail = function() {
		...
		$.post(interfaceApi, data, function(result) {
			...
			if (result.messageCode == 1) {
				...
				$('<p>活动时间：' + result.resultObject.startDate + '~' + result.resultObject.endDate + '</p><p>活动地点：' + result.resultObject.place + '</p><p>人数：' + result.resultObject.infactNum + '/' + result.resultObject.planNum + '</p>').appendTo(".banner");
				...
				if (JSON.parse(eval($.cookie(managerMemory))) != null) {
					if (JSON.parse(eval($.cookie(managerMemory))).mobile == result.resultObject.createrId) {
						$(".judge").hide();
					}

				}

				if (result.resultObject.infactNum == result.resultObject.planNum) {
					$(".judge").hide();
				}
				...
				if (JSON.parse(eval($.cookie(managerMemory))) != null) {
					...
					if (result.resultObject.createrId == JSON.parse(eval($.cookie(managerMemory))).mobile) {
						...
						//循环参与活动的人员
						$.each(result.resultObject.actionUsers, function(i, item){
							if(item.userId==JSON.parse(eval($.cookie(managerMemory))).mobile){
								$("#participation").text("退出活动");
							}
						});
					}
		...
		$(".banner").delegate(".picture img", "click", function () {
        ...
        $(".top").delegate(".delete_activity", "click", function() {
				$(".tan1").show();
				$(".mask").show();
			});
		...
		$(".banner").delegate(".thumbUp", "click", function() {
			...
			$.post(interfaceApi, data, function(result) {
				...
				if (result.messageCode == 1) {
					getBomBbox(result.message);
					setTimeout(function() {
						location.href = "personal_activities_details.html?id=" + GetQueryString('id');
					}, 500)
 ...
 $(".images").delegate(".delete", "click", function () {
	...
	this.my_activity = function() {
		isLogin();
		var isCookie = JSON.parse($.cookie("IsMyActivity"));
		if (isCookie == null) {
			getBomBbox("↑上拉加载更多");
			$.cookie("IsMyActivity", 1, {
				expires: 3650,
				path: "/"
			});
		}
		...
		$.post(interfaceApi, data, function(result) {
			...
			if (result.messageCode == 1) {
				...
				$.each(result.resultObject, function(i, item) {
					$('<div class="topic"><a href="personal_activities_details.html?id=' + item.id + '"><div class="topic_img"><img src="' + imgdomain + item.createrImage + '"/></div><div class="topic_content"><p class="title">' + item.name + '</p><p><span class="nickname">' + item.createrNickName + '</span><span>' + item.createTime + '</span></p></div></a></div>').appendTo(".list");
